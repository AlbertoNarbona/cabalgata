...
export const pagosService = {
  ...
  async getPagos(tipo: string): Promise<Pago[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/tipoRecibo/Pagos/${tipo}`);
      if (!response.ok) {
  ...
  async getPagosByRecibo(reciboId: number, tipo: string): Promise<Pago[]> {
    try {
      const pagos = await this.getPagos(tipo);
  ...
  async getTotalPagadoPorRecibo(reciboId: number, tipo: string): Promise<number> {
    try {
      const pagos = await this.getPagosByRecibo(reciboId, tipo);
  ...
  async isReciboPagado(reciboId: number, importeRecibo: number, tipo: string): Promise<boolean> {
    try {
      const totalPagado = await this.getTotalPagadoPorRecibo(reciboId, tipo);
